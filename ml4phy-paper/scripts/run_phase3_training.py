#!/usr/bin/env python3
"""Run one frozen Phase 3 neural training job on its exact subset."""
...
def monitor_gpu_memory(pid: int, stop: threading.Event, observed: dict) -> None:
    """Poll per-process VRAM without altering the training process."""
    while not stop.wait(0.25):
        ...
        for line in output.splitlines():
            ...
            if len(fields) == 2 and fields[0] == str(pid):
                observed["peak_mib"] = max(observed["peak_mib"], int(fields[1]))
...
def main() -> None:
    ...
    parser.add_argument("--subset-id", choices=SUBSET_IDS, required=True)
    ...
    run_dir = root / "runs/phase3/training" / args.run_id
    ...
    if config["status"] != "frozen" or protocol["status"] != "frozen_approved_not_started":
        parser.error("Phase 3 protocol is not approved and frozen")
    validate_dependency(source_root, compatibility)
    subset = next(item for item in config["subset_inputs"] if item["subset_id"] == args.subset_id)
    subset_path = repo / subset["logical_path"]
    if not subset_path.is_file() or sha256_file(subset_path) != subset["sha256"]:
        parser.error("Phase 3 subset HDF5 is missing or has the wrong hash")
    count, observed_identity_hash = identity_hash(subset_path)
    if count != subset["nominal_unique_events"] or observed_identity_hash != subset["identity_sha256"]:
        parser.error("Phase 3 subset count or identity hash mismatch")
    ...
    if worktree:
        parser.error("Phase 3 training requires a clean worktree: " + "; ".join(worktree))
    ...
    budget = int(subset["nominal_unique_events"])
    resolved["name"] = f"ml4ps_phase3_{args.subset_id}_{args.architecture}_seed{args.training_seed}"
    ...
    if resolved["training"]["n_steps"] != 3000 or resolved["training"]["batch_size"] != 16:
        parser.error("Phase 3 requires the frozen 3,000-step, batch-16 schedule")
    ...
    if resolved.get("device") != "cuda":
        parser.error("Phase 3 training requires CUDA")
    ...
    with log_path.open("x") as log_stream:
        ...
        observed_gpu = {"available": True, "peak_mib": 0}
        stop_monitor = threading.Event()
        monitor = threading.Thread(
            target=monitor_gpu_memory,
            args=(process.pid, stop_monitor, observed_gpu),
            daemon=True,
        )
        monitor.start()
    ...
    record["peak_gpu_memory_mib"] = observed_gpu["peak_mib"] if observed_gpu["available"] else None
