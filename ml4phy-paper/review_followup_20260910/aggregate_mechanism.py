...
The decoder-gate contrast is positive for both endpoints in all three seed means (Peaks +12.5, +25.0, +35.0 points; Continuum +4.5, +8.7, +8.0 points). This supports a useful role for the energy-dependent decoder gate in this one-subset slice. The attention contrast is near zero and changes sign across seeds, so a necessary benefit from density-adaptive attention is not supported. The direct-input control is mixed: `global_both` improves mean Peaks C2 by {direct_peak:+.1f} points but changes Continuum C2 by {direct_cont:+.1f} points relative to `density_free_global`; it does not improve the joint endpoint unconditionally.
